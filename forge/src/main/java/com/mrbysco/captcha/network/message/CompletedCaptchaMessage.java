...
public class CompletedCaptchaMessage {
	private final CompleteCaptchaData data;

	public CompletedCaptchaMessage(CompleteCaptchaData data) {
		this.data = data;
	}
	...
	public static CompletedCaptchaMessage decode(final FriendlyByteBuf packetBuffer) {
		return new CompletedCaptchaMessage(CompleteCaptchaData.decode(packetBuffer));
	}
	...
	public void handle(Supplier<Context> context) {
		...
		ctx.enqueueWork(() -> {
			if (ctx.getDirection().getReceptionSide().isServer() && ctx.getSender() != null) {
				CompleteCaptcha.complete(ctx.getSender().getUUID(), this.data.code()).run();
			}
		});
