...
public class ImageScreen extends CaptchaScreen {
	...
	public ImageScreen(String code, int maxCompletionTime) {
		super(new TranslatableComponent("captcha.image.screen"), code, maxCompletionTime);
	}
	...
	protected void init() {
		...
		this.message = MultiLineLabel.create(this.font, List.of(
				TextComponent.EMPTY,
				new TranslatableComponent("captcha.image.screen"),
				TextComponent.EMPTY,
				new TranslatableComponent("captcha.image.none")
		));
		...
		this.addRenderableWidget(button =
				new Button(this.width / 2 - 60, startY + (SQUARE_SIZE * 4) + 36, 120, 20,
						new TranslatableComponent("captcha.image.skip"), (button) -> checkSquares())
		);
	}
	...
	private void changeImage(boolean reset) {
		...
		if (reset) {
			error = MultiLineLabel.create(this.font, List.of(
					new TranslatableComponent("captcha.text.failed").withStyle(ChatFormatting.RED),
					new TranslatableComponent("captcha.text.failed2").withStyle(ChatFormatting.RED)
			));
		}
		this.message = MultiLineLabel.create(this.font, List.of(
				new TranslatableComponent("captcha.image.screen"),
				TextComponent.EMPTY,
				new TextComponent(object).withStyle(ChatFormatting.GOLD),
				TextComponent.EMPTY,
				new TranslatableComponent("captcha.image.none")
		));
	}
			...
			for (int y = 0; y < 4; y++) {
				...
				if (toggleButton != null) {
					if (toggleButton.isClicked()) {
						this.button.setMessage(new TranslatableComponent("captcha.image.verify"));
						return;
					}
				}
			}
		}
		this.button.setMessage(new TranslatableComponent("captcha.image.skip"));
	}
