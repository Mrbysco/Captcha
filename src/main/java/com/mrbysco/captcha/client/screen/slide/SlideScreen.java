...
public class SlideScreen extends CaptchaScreen {
	...
	public SlideScreen(String code, int maxCompletionTime) {
		super(new TranslatableComponent("captcha.slide.screen"), code, maxCompletionTime);
		this.resetPuzzle();
	}
	...
	protected void init() {
		...
		this.addRenderableWidget(this.slider = new PuzzleSlider(this.width / 2 - 100, this.height / 2 + 100, 200, 20,
				TextComponent.EMPTY, TextComponent.EMPTY, 0, 84, 0, 0.1, 0, (slider) -> {
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		...
		this.fillGradient(poseStack, pieceX, pieceY, pieceX + 32, pieceY + 32, 0xA6000000, 0xA6000000);

		RenderSystem.setShader(GameRenderer::getPositionTexShader);
		RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		RenderSystem.setShaderTexture(0, IMAGE);
		int sliderX = (this.width / 2 - 100) + (int) (this.slider.getValue() * 2);
		blit(poseStack, sliderX, pieceY, this.puzzleX * 2 - 32, this.puzzleY * 2 - 32, 32, 32, 200, 200);
		this.fillGradient(poseStack, sliderX, pieceY, sliderX + 32, pieceY + 32, 0x1AFFFFFF, 0x1AFFFFFF);
	}
	...
	public void renderBackground(PoseStack poseStack) {
		...
		RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		RenderSystem.setShaderTexture(0, IMAGE);
		blit(poseStack, this.width / 2 - 100, this.height / 2 - 100, 0, 0, 200, 200, 200, 200);
	}
}